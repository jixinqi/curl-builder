...
class builder_quictls_openssl(builder_base):
    def __init__(self):
        super().__init__()
        self.__project_dir = pathlib.Path(__file__).parent.parent.parent
        self.__module_dir  = self.__project_dir / "third_party" / "quictls_openssl"
        if(
    ...
    def __run_make(self):
        build_dir   = self.__project_dir / "__build"   / "quictls_openssl"
        install_dir = self.__project_dir / "__install" / "quictls_openssl"
...
def main():
    builder = builder_quictls_openssl()
    builder.build()
